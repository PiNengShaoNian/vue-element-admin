...
const mutations = {
  ...
  DEL_OTHER_CATCHED_VIEWS: (state, view) => {
    const index = state.cachedViews.indexOf(view.name)
    if (index > -1) {
...
const actions = {
  delAllCatchedViews({ commit }, state) {
  ...
  delOtherViews({ dispatch, state }, view) {
    return new Promise(resolve => {
      ...
      dispatch('delOtherCatchedViews', view)
  ...
  delOtherCatchedViews({ commit, state }, view) {
    return new Promise(resolve => {
      commit('DEL_OTHER_CATCHED_VIEWS', view)
      resolve([...state.cachedViews])
    })
  },
